...
class PersistentLCARunner:
    ...
    UMFPACK is loaded via ``_patch_umfpack_import`` (it stubs the removed
    ``numpy.testing.Tester`` symbol so ``scikits.umfpack`` imports on
    numpy ≥ 1.25 — raw import fails), and it is present in BOTH the conda dev
    env AND the frozen desktop sidecar: ``mapper-desktop.spec`` applies the same
    shim at build time and bundles the ``__umfpack`` extension + its SuiteSparse
    dylibs (they live in ``$CONDA_PREFIX/lib``, outside the package, so the
    PyInstaller collectors return empty and they are added by explicit path).
    Verified inside the frozen process: ``_UMFPACK_OK`` is True and
    ``factorizations`` stays at the number of premise dbs.

    Measured on ecoinvent-3.10 premise dbs (≈ 30 k × 30 k): UMFPACK factor
    ≈ 4.5 s, back-sub ≈ 0.035 s. The Danish fleet case — 3 LCI scenarios ×
    26 years × 25 indicators — completes in **under a minute** (6 anchor-db
    factorizations, everything else back-substitution), down from ≈ 54 minutes
    before the extension was bundled.

    ``spsolve``-per-call remains as a fallback for environments without the
    extension, where the reuse is LOST and each call re-solves the full system.
    A *pure* SuperLU ``splu`` reuse-fallback was profiled and rejected:
    factorizing a premise technosphere with SuperLU measures ≈ 120 s/db AND
    shifts scores vs UMFPACK by ≈ 3e-7 (SuperLU vs UMFPACK LU), so it would
    trade a large slowdown for a correctness change. Bundling the real UMFPACK
    extension is the fix — the frozen build matches the dev UMFPACK answer to
    ≈ 7e-10 (an openBLAS-build difference between the two environments, NOT a
    solver change).
