...
def test_fresh_config_carbon_budget_defaults():
    """A fresh AESA config defaults to IPCC AR6 2.0°C / 50th pct (1150 Gt from
    2025) × SSP1-2.6 (a temperature-CONSISTENT ~2°C pathway), allocated over the
    full century (end_year 2100). ``build_carbon_budget()`` (no args) is exactly
    what ``GET /aesa/defaults`` serves as ``default_carbon_budget``, so this
    locks the fresh-start defaults.

    The temperature default is a UX choice — 2°C/50 × a ~2°C pathway preserves
    the comparative SR gradient across 2025–2050; the 1.5°C budget saturates
    inherently by ~2033–2040 under any pathway. SSP1-2.6 (not SSP2-4.5) avoids a
    mitigation-gap default pairing. end_year is the BUDGET ALLOCATION horizon
    (remaining / (end_year - t)), NOT the study/SR-timeline window (DSM-driven);
    it must stay 2100 (5AR)."""
    from mapper.core.aesa_engine import build_carbon_budget
...
def test_get_defaults_surfaces_fresh_carbon_budget():
    ...
    defaults (the only no-arg build_carbon_budget caller) — 2.0°C/50 × SSP1-2.6."""
    import asyncio
