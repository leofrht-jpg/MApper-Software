...
export function DimensionsEditor({ dimensions, onChange, timeHorizon }: DimensionsEditorProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
      ...
      <Button variant="ghost" onClick={addDim} style={{ alignSelf: 'flex-start', color: 'var(--mod-mfa)' }}>
...
function SingleDimensionEditor({ dim, onChange, onDelete }: SingleDimensionEditorProps) {
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      let parsed: string[]
      if (isExcel) {
        const res = await parseLabelFile(file)
        parsed = res.labels
      } else {
        const text = await file.text()
        const lines = text.split(/\r?\n/).map((l) => l.trim()).filter(Boolean)
        if (lines.length === 0) { showFlash('Empty file', true); return }
        const first = lines[0].split(',')[0].trim()
        const body = first.toLowerCase() === dim.name.toLowerCase() ? lines.slice(1) : lines
        parsed = body.map((l) => l.split(',')[0].trim()).filter(Boolean)
      }
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <button
              ...
              style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--mod-mfa)', fontSize: 'var(--text-xs)', fontWeight: 500, display: 'inline-flex', alignItems: 'center', gap: 4, padding: 0 }}
            >
        ...
        <div style={{
          ...
          {dim.labels.map((l) => (
            <span key={l} style={{
              ...
              backgroundColor: 'color-mix(in srgb, var(--mod-mfa) 12%, transparent)',
              color: 'var(--mod-mfa)', fontSize: 'var(--text-xs)', fontWeight: 500,
            }}>
              {l}
