...
export function SurvivalConfigurator({ onClose }: SurvivalConfiguratorProps) {
  const activeSystem = useMFAStore((s) => s.activeSystem)
  const systemState = useMFAStore((s) => s.systemState)
  const setSurvival = useMFAStore((s) => s.setSurvival)
    ...
    >
      <div style={{
        ...
        <div style={{ padding: 'var(--space-6)', display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
          ...
          <div style={{ height: 240, padding: 'var(--space-4)', backgroundColor: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-md)' }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={preview} margin={{ top: 8, right: 30, bottom: 8, left: 0 }}>
                ...
                />
                <Line yAxisId="left" type="monotone" dataKey="survival_rate" name="S(a)" stroke="var(--mod-mfa)" strokeWidth={2} dot={false} isAnimationActive={false} />
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            ...
            <Button variant="primary" onClick={handleSave} disabled={saving} style={{ backgroundColor: 'var(--mod-mfa)' }}>
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <input
          ...
          style={{ flex: 1, accentColor: 'var(--mod-mfa)' }}
        />
        <input
          type="number"
          min={min}
